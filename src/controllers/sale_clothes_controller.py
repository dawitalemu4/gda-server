...
class sale_clothes_queries(viewsets.ViewSet):
    ...
    def create_clothing(data):
        try:
            ...
            return Response("Created Sucessfully", status=200)
        except create_clothing.DoesNotExist:
            logging.warning("Could not insert clothing into database")
            return Response("Could not insert clothing into database", status=400)
        except Exception as e:
            logging.warning(f"Server error: {e}")
            return Response(f"Server error: {e}", status=500)
    ...
    def update_clothing(id, data):
        try:
            ...
            return Response("Update Sucessfully", status=200) 
        except update_clothing.DoesNotExist:
            logging.warning("Could not complete this update request.")
            return Response("Could not complete this update request.", status=400)
        except Exception as e:
            logging.warning(f"Server error: {e}")
            return Response(f"Server error: {e}", status=500)
    ...
    def delete_clothing(id):
        try:
            ...
            return Response("Could not find clothing with this id to delete.", status=400)
        except Exception as e:
            logging.warning(f"Server error: {e}")
            return Response(f"Server error: {e}", status=500)